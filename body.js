...
export const Body = () => {
  const [oldlist, newlist] = useState(Recipe); 
    ...
    <>
      <div className="maindiv">
        <div className="imagediv">
          <img
            className="image"
            src="https://img.freepik.com/premium-vector/flat-design-people-cooking-food-kitchen_108061-1087.jpg?w=740"
            alt="Cooking Illustration"
          />
        </div>
        <div className="headingdiv">
          <h1>
            <i>Unveiling the stories<br /> behind every ingredient</i>
          </h1>
        </div>
      </div>
          ...
          <button
            ...
            onClick={() => {
              const filterRecipe = Recipe.filter(
                (prod) => prod.difficulty === "Easy"
              );
              newlist(filterRecipe);
            }}
          >
            Easy-Recipe
          </button>
      ...
      <div className="mapdiv">
        {oldlist.length > 0 ? (
          oldlist.map((list) => (
            <RecipeCard key={list.id} ObjList={list} />
          ))
        ) : (
